...
export class Content extends EE {
  ...
  async getFromId(id, guid) {
    ...
    const fields = await this.cache.get(`fields:${id}`, () => ChannelData
      .find({
    ...
    return await this.cache.get(guid, () => ChannelData
      .findOne({
        where: { entry_id: Number(id) },
  ...
  async getFromPublishedId(id, guid) {
    ...
    const exp_channel_fields = this.createFieldObject(fields);
    return await this.cache.get(`future:${guid}`, () => ChannelData
      .findOne({
        where: { entry_id: Number(id) },
        attributes: ["entry_id", "channel_id", "site_id"].concat(fields),
        include: [
  ...
  async getContentFromMatrix(entry_id, name, field_id) {
    ...
    return await this.cache.get(this.cache.encode(query), () => ChannelData
      .find({
        where: { entry_id },
        attributes: ["entry_id"],
        include: [
  ...
  async getEntryFromFieldValue(value, field_id, channel_id) {
    ...
    return this.cache.get(this.cache.encode(vars), () => ChannelData
      .findOne({
        attributes: ["entry_id"],
  ...
  async findByChildId(entry_id) {
    return this.cache
      .get(`${entry_id}:Playa`, () => Playa.findOne({
  ...
  async getLiveStream() {
    return this.getIsLive().then(x => !x ? { isLive: false } : x).then((
      { isLive },
    ) => {
  ...
  async findByTagName({ tagName, includeChannels }, { limit, offset }, cache) {
    ...
    return this.cache
      ...
      .then(x => x.slice(offset, limit + offset)) // XXX find how to do this in the query?
      .then(this.getFromPublishedIds)
      .then(x => x.filter(y => !!y));
  }
  ...
  async findByTags(
    ...
    return this.cache
      ...
      .then(x => sampleSize(x, limit)) // XXX make a WileySort for this instead of random stuff // return x.slice(offset, limit + offset); // XXX find how to do this in the query?
      .then(this.getFromPublishedIds)
      .then(x => x.filter(y => !!y));
  }
