...
import { Person as PersonTable, PersonAlias, PhoneNumber as PhoneNumberTable } from "./tables";
import {
  AttributeValue,
  Attribute,
} from "../system/tables";
...
export class Person extends Rock {
        ...
        GroupLocation.find({
          ...
          include: [
            { model: Location.model },
            {
              ...
              attributes: [],
              where: { GroupTypeId: 10 }, // Family
              include: [{ model: GroupMember.model, where: { PersonId: `${id}` }, attributes: [] }],
            },
          ],
  ...
  async getSecurityRoles(id) {
    return this.cache.get(`${id}:GroupMemberId`, () =>
      Group.find({
        attributes: ["Name", "Id"],
        where: { GroupTypeId: 1 },
        include: [{ model: GroupMember.model, where: { PersonId: `${id}` }, attributes: [] }],
  ...
  async setPersonAttribute({ attributeId, attributeValue }, person) {
    ...
    // Try to get the current attribute values for the
    // given attribute id.
    const theAttributeValues = await AttributeValue.find({
      where: {
    ...
    if (theAttributeValues && theAttributeValues.length > 0) {
      await AttributeValue.patch(attributeID, entityId, attributeValue);
    }
