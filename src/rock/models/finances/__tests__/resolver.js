...
describe("Query", () => {
  describe("SavedPayment", () => {
    it("finds by person alias", () => {
      ...
      resolver.Query.savedPayments(null, { limit: 3, cache: true, skip: 1 }, {
        models,
        person,
      });
  ...
  describe("AccountFromCashtag", () => {
    it("returns an account from a cashtag", async () => {
      ...
      resolver.Query.accountFromCashTag(null, { cashTag: "$tesla" }, {
        models,
      });
      expect(models.FinancialAccount.find).toBeCalledWith({
        IsActive: true,
...
describe("Mutation", () => {
  describe("syncTransactions", () => {
    it(
      "has a `syncTransactions` which passes args to Transaction.syncTransactions",
      () => {
        const models = {
          Transaction: {
  ...
  describe("validate", () => {
    it("loads gatewayDetails and validates a card", async () => {
      ...
      expect(models.Transaction.loadGatewayDetails).toBeCalledWith("nmi");
      expect(models.SavedPayment.validate).toBeCalledWith({ token: "token" }, {
        nmi: true,
      });
    });
