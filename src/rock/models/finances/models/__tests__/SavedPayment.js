...
describe("removing a saved payment", () => {
  ...
  it(
    "doesn't try to delete the payment in NMI if response is missing a code",
    async () => {
      const id = 1;
      const Local = new SavedPayment({ cache: mockedCache });
      SavedPaymentTable.find.mockReturnValueOnce([{ Id: id }]);
  ...
  it(
    "calls the nmi method with the passed security key and the code of the payment",
    async () => {
      const id = 1;
      const Local = new SavedPayment({ cache: mockedCache });
      SavedPaymentTable.find.mockReturnValueOnce([
...
describe("validate", () => {
  ...
  it("calls charge with the needed data", async () => {
    ...
    const result = await Local.validate({ token: "token" }, {
      SecurityKey: "foo",
    });
