...
it("builds an xml object from the data provided", () => {
  ...
  return nmi(payload, { APIUrl: "https://example.com" })
    .then((x) => {
...
it("errors if request takes longer than a minute", () => {
  ...
  return nmi(payload, { APIUrl: "https://example.com" })
    .catch((err) => {
      ...
      expect(err.message).toMatch(/The request to our payment process took longer than expected./);
    });
});
