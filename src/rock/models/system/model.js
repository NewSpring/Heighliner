...
export class Rock extends Heighliner {
  ...
  async getAttributeFromId(id) {
    return this.cache.get(`${id}:getAttributeFromId`, () =>
      AttributeModel.findOne({
        where: { Id: id }
      })
    );
  }
  ...
  async getAttributeFromKey(key, EntityId) {
    ...
    return this.cache.get(`${key}:${EntityId}:getAttributeFromKey`, () =>
      AttributeModel.findOne({
        where
      })
    );
  ...
  async getAttributeValuesFromAttributeId(id, context, EntityId) {
    ...
    return this.cache.get(
      `${id}:${EntityId}:getAttributeValuesFromAttributeId`,
      () =>
        AttributeValueModel.find({
          where,
          include: [
            {
  ...
  async getAttributeValuesFromId(id, context) {
    return this.cache.get(`${id}:getAttributeValuesFromId`, () =>
      AttributeValueModel.findOne({
        where: { Id: id },
        include: [
          {
            model: AttributeModel.model,
  ...
  async getAttributesFromEntity(id, key, EntityTypeId) {
    ...
    return this.cache
      .get(`${id}:${key}:getAttributesFromEntity`, () =>
        AttributeModel.find({
          where,
          include: [
      ...
      .then(y =>
        Promise.all(
          y.map(async x => {
            ...
            return await this.getDefinedValuesByTypeId(
              definedTypeId[0].Value
            ).then(x => ({
              ...x,
              EntityId: id
            }));
  ...
  async getAttributesFromId(id) {
    return this.cache.get(`${id}:getAttributeValues`, () =>
      AttributeModel.findOne({
        where: { Id: id },
        include: [
          { model: FieldTypeModel.model },
  ...
  async getAttributeValueFromMatrix(key, knownKey, knownValue, desiredKey) {
    return this.cache
      .get(`${key}:${knownKey}:${knownValue}:${desiredKey}:MatrixValue`, () =>
        CommunicationTable.db
          .query(
            `
        SELECT
            desiredValue.*
