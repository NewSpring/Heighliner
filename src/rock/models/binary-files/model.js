...
import {
  BinaryFile as BinaryFileTable, // Location as LocationTable, // XXX move to its own model
} from "./tables";
...
export class BinaryFile extends Rock {
  ...
  async getFromGuid(Guid) {
    return this.cache.get(`${Guid}:BinaryFileGuid`, () => BinaryFileTable
      .findOne({
        where: { Guid },
      })
  ...
  // async getFromPerson
  async find(query) {
    return this.cache.get(this.cache.encode(query), () => BinaryFileTable
      .find({
        where: query,
        attributes: ["Id"],
      })
